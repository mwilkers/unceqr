...
//	fprintf(stderr, "%s\tpos=%d\tend=%d\t(%d,%d,%d)\n", bam1_qname(b), pos, s->end, s->k, s->x, s->y);
	...
	{ // collect pileup information
		...
		//mdw ##
			int qbeg,qend;
	 		qbeg = tpos2qposB(&p->b->core, bam1_cigar(p->b));
 			int alignedQPosBeg = p->qpos - qbeg + 1; // 0-based.
	 		qend = tpos2qposE(&p->b->core, bam1_cigar(p->b));
 			int alignedQPosEnd = (p->b->core.l_qseq - qend) - (p->qpos); // length to end.
			p->alignedQPosBeg=alignedQPosBeg;
			p->alignedQPosEnd=alignedQPosEnd;
			...
			bam_aux_get_set(p->b, &nmd, &ih, &zf, &mdF);
			...
			int indelLen=0;
			int indelCount=0;
			tpos2qposIndelCount(&p->b->core, bam1_cigar(p->b), &indelLen, &indelCount);
			
			if(mdF==1){
				nmd = nmd - indelLen + indelCount;
			}else{
				nmd = nmd + indelCount;
			}
...
int tpos2qposB(const bam1_core_t *c, const uint32_t *cigar)
{

	// first position
	int op = cigar[0] & BAM_CIGAR_MASK;
	int l = cigar[0] >> BAM_CIGAR_SHIFT;
	if(	op == BAM_CSOFT_CLIP){
		return l;  // 1-based
	}else{
		return 0; // #no soft clipping.
	}
}


int tpos2qposE(const bam1_core_t *c, const uint32_t *cigar)
{

	// last position
	int op = cigar[c->n_cigar -1] & BAM_CIGAR_MASK;
	int l = cigar[c->n_cigar - 1 ] >> BAM_CIGAR_SHIFT;
	if(	op == BAM_CSOFT_CLIP){
		return l; // 1-based
	}else{
		return 0; // return 0 for no soft clipping
	}
}


int tpos2qposIndelCount(const bam1_core_t *c, const uint32_t *cigar, int *indelLen, int *indelCount)
{
	int k;
	...
	// last position
	  ...
	  if(	op == BAM_CINS || op == BAM_CDEL){
	        *indelLen += cigar[k] >> BAM_CIGAR_SHIFT;
		*indelCount += 1;
	  }
        }
	return;
