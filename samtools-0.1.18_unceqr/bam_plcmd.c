...
typedef struct {
	...
	// MDW
	int trimEnd, maxNM, maxIH, flagFilter,min_mqToCount,maxrepC;
...
int bam_mpileup(int argc, char *argv[])
{
	...
	// MDW
	mplp.min_mq=-1000; // set as zero for everything.
	mplp.trimEnd=-1; mplp.maxIH=10000; mplp.maxNM=10000;
	mplp.flagFilter=0;
	mplp.min_mqToCount=-10000;
	mplp.maxrepC=10000;
	// end MDW

	mplp.flag = MPLP_NO_ORPHAN | MPLP_REALN;
	while ((c = getopt(argc, argv, "Agf:r:l:M:q:Q:uaRC:BDSd:L:b:T:H:N:K:Z:P:o:e:h:Im:F:EG:6Os")) >= 0) {
		switch (c) {
		case 'f':
		...
		// MDW
		case 'q': mplp.min_mqToCount = atoi(optarg); break;
		case 'T': mplp.trimEnd = atoi(optarg); break;
		case 'H': mplp.maxIH = atoi(optarg); break;
		case 'N': mplp.maxNM = atoi(optarg); break;
		case 'K': mplp.flagFilter = atoi(optarg); break;
		case 'Z': mplp.maxrepC = atoi(optarg); break;
		// end MDW
